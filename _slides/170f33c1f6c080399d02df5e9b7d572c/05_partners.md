---
project: 170f33c1f6c080399d02df5e9b7d572c
order: 5
title: partners
---
# Supporters & Partnerships

## Institutional Partners

- **European Commission** (collaborative discussions on blockchain strategy)
- **European Parliament** (consultation on legislative processes)
## Industry Collaborators

- **Ledger** (hardware wallet and security experts)
- **Aave** (leading DeFi protocol)
- **Unstoppable Domains** (Web3 domain provider)
- **Global Blockchain Business Council (GBBC)** (international advocacy group)
## Community Allies

- **Academic institutions** for research and knowledge-sharing
- **Startup accelerators** that foster new talent and cutting-edge solutions
- **Open-source communities** contributing to decentralized frameworks
> Our diverse network of supporters spans government agencies, blockchain pioneers, and grassroots organizations. Together, we strive to shape a regulatory environment that promotes fair competition, consumer protection, and technological advancement.

